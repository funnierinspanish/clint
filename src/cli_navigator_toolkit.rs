...
pub fn run_webpage_generator(input_json: &std::path::Path, output_path: &std::path::Path) {
    ...
    for entry in std::fs::read_dir(src_path).expect("Failed to read directory") {
        ...
        let dest_file_path =
            output_path.join(src_file_path.file_name().expect("Failed to get file name"));
