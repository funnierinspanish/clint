...
fn main() {
    ...
    match &cli.command {
        ...
        Some(Commands::UniqueKeywords { input_json, output_path, format }) => {
          ...
          let output_path = match output_path {
            Some(path) => path,
            None => &current_dir().expect("Failed to get current directory").join(format!("{}-keywords.json", input_json_file_name.unwrap_or("output"))),
          };
        ...
        Some(Commands::Summary { input_json, output_path, format }) => {
          let input_json = match input_json {
            Some(path) => path,
            None => {
              println!("No input JSON file provided.");
          ...
          let out_path = match output_path {
            Some(path) => path,
            None => &current_dir().expect("Failed to get current directory").join(format!("{}-summary.json", input_json_file_name.unwrap_or("output"))),
          };
          let out_path_extension = match output_path {
            Some(path) => path.extension().expect("Failed to get extension").to_str(),
            None => None,
        ...
        Some(Commands::Webpage { input_json, output_dir, force }) => {
          let input_json = match input_json {
            Some(path) => path,
            None => {
              println!("No input JSON file provided.");
          ...
          match output_dir {
            ...
            None => {
              ...
              let output_path = current_dir.join(format!("./out/{}-webpage", input_json_file_name.unwrap_or("output")));
