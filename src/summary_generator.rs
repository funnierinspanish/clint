...
pub fn generate_summary(path: &PathBuf) -> Result<CLISummary, Box<dyn std::error::Error>> {
  ...
  let unique_keywords_count = unique_command_count + unique_subcommand_count + unique_short_flag_count + unique_long_flag_count;

  return Ok(CLISummary {
...
fn extract_data(path: &PathBuf) -> Option<CLIKeywords> {
  ...
  let base_program = json.get("name").and_then(|v| v.as_str()).unwrap_or_default().to_string();
    ...
    if let Some(command_map) = node.get("COMMAND").and_then(|v| v.as_object()) {
        for (subcmd_name, subcmd_obj) in command_map {
            ...
            if let Some(grandchildren) = subcmd_obj.get("children") {
                walk_commands_recursively(subcmd_name, grandchildren, subcommands, short_flags, long_flags);
            }
        }
    }
