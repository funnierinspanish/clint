...
#[derive(Debug, Clone)]
pub enum ChangeType {
    CommandAdded { parent: String, command: String },
    CommandRemoved { parent: String, command: String },
    FlagAdded { command: String, flag: String },
    FlagRemoved { command: String, flag: String },
    FlagDescriptionChanged { command: String, flag: String, old_desc: String, new_desc: String },
    FlagDataTypeChanged { command: String, flag: String, old_type: Option<String>, new_type: Option<String> },
}
...
impl ChangeType {
    pub fn format(&self) -> String {
        match self {
            ...
            ChangeType::FlagDescriptionChanged { command, flag, old_desc, new_desc } => {
                format!("~ Modified flag: {} (command: {})\n    Description changed:\n      Before: \"{}\"\n      After:  \"{}\"", 
                       flag, command, old_desc, new_desc)
            }
            ChangeType::FlagDataTypeChanged { command, flag, old_type, new_type } => {
...
pub fn compare_json_structures(from_path: &Path, to_path: &Path) -> Result<Vec<ChangeType>, Box<dyn std::error::Error>> {
...
fn compare_commands_json(from: &Value, to: &Value, parent_path: &str, changes: &mut Vec<ChangeType>) {
    // Get command maps from both structures
    let from_commands = from.get("children")
        .and_then(|c| c.get("COMMAND"))
        .and_then(|c| c.as_object());
    
    let to_commands = to.get("children")
        .and_then(|c| c.get("COMMAND"))
        .and_then(|c| c.as_object());
    ...
    if let (Some(from_cmds), Some(to_cmds)) = (from_commands, to_commands) {
        for command_name in from_set.intersection(&to_set) {
            if let (Some(from_cmd), Some(to_cmd)) = (
                from_cmds.get(command_name),
                to_cmds.get(command_name)
            ) {
...
fn compare_flags_json(from: &Value, to: &Value, command_path: &str, changes: &mut Vec<ChangeType>) {
    ...
    let from_flag_map: HashMap<String, &Value> = from_flags.iter()
        .map(|f| (get_flag_signature(f), *f))
        .collect();
    
    let to_flag_map: HashMap<String, &Value> = to_flags.iter()
        .map(|f| (get_flag_signature(f), *f))
        .collect();
    ...
    for (key, from_flag) in &from_flag_map {
        if let Some(to_flag) = to_flag_map.get(key) {
            ...
            let from_desc = from_flag.get("description").and_then(|v| v.as_str()).unwrap_or("");
            let to_desc = to_flag.get("description").and_then(|v| v.as_str()).unwrap_or("");
            ...
            // Compare data types
            let from_type = from_flag.get("data_type").and_then(|v| v.as_str()).map(|s| s.to_string());
            let to_type = to_flag.get("data_type").and_then(|v| v.as_str()).map(|s| s.to_string());
...
pub fn compare_zod_directories(from_dir: &Path, to_dir: &Path) -> Result<Vec<ChangeType>, Box<dyn std::error::Error>> {
    let mut changes = Vec::new();
...
fn analyze_typescript_changes(
    ...
    let from_flag_map: HashMap<String, TypeScriptFlag> = from_flags.into_iter()
        .map(|f| (f.get_signature(), f))
        .collect();
    
    let to_flag_map: HashMap<String, TypeScriptFlag> = to_flags.into_iter()
        .map(|f| (f.get_signature(), f))
        .collect();
...
fn extract_flags_from_typescript(content: &str) -> Vec<TypeScriptFlag> {
    ...
    if let Some(start_pos) = content.find("_FLAGS: CommandFlag[] = [") {
        if let Some(bracket_pos) = content[start_pos..].find("= [") {
...
fn get_ts_files_recursive(dir: &Path, base_dir: &Path, files: &mut Vec<String>) -> Result<(), Box<dyn std::error::Error>> {
    if dir.is_dir() {
